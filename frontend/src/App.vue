...
<template>
  ...
  <main v-else class="app-shell">
    ...
    <section class="content">
      <header v-if="view !== 'relatorios'" class="topbar">
        <div>
      ...
      <p v-if="error && view !== 'relatorios'" class="error">{{ error }}</p>
      ...
      <section v-if="view === 'vendas'" class="panel table-panel"><h2>Ultimas vendas</h2><div class="table"><div v-for="venda in vendas" :key="venda.id" class="row"><span>#{{ venda.id }}<small>{{ saleDate(venda.dataVenda) }}</small></span><span>{{ venda.clienteNome || 'Sem cliente' }}</span><span>{{ vendaStatus(venda.statusVenda) }}</span><strong>{{ money(venda.valorTotal) }}</strong></div></div></section>
